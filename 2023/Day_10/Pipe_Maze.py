...
The landscape here is alien; even the flowers and trees are made of metal. As you stop to admire some metal grass, you notice something metallic scurry 
away in your peripheral vision and jump into a big pipe! It didn't look like any animal you've ever seen; if you want a better look, you'll need to get 
ahead of it.

Scanning the area, you discover that the entire field you're standing on is densely packed with pipes; it was hard to tell at first because they're the 
same metallic silver color as the "ground". You make a quick sketch of all of the surface pipes you can see (your puzzle input).
...
those pipes connect to, and so on. Every pipe in the main loop connects to its two neighbors (including S, which will have exactly two pipes connecting 
...
|F--J
LJ.LJ
If you want to get out ahead of the animal, you should find the tile in the loop that is farthest from the starting position. Because the animal is in 
the pipe, it doesn't make sense to measure this by direct distance. Instead, you need to find the tile that would take the longest number of steps along 
...
14567
23...
Find the single giant loop starting at S. How many steps along the loop does it take to get from the starting position to the point farthest from the 
starting position?
...
.L--J.L--J.
...........
The above loop encloses merely four tiles - the two pairs of . in the southwest and southeast (marked I below). The middle . tiles (marked O below) are 
not in the loop. Here is the same loop again with those regions marked:
...
def search(start, input):
    
    visited = []
    q.append(start)

    while True:
        # current_pos = q.popleft()   # uncomment for BFS (Part 1)  ....need this to find farthest point from start
        current_pos = q.pop()   # uncomment for DFS (Part 2) ....need this to find a contiguous path
...
def steps(input):
    ...
    answer = search(start, input)
    max_steps = len(answer) / 2
    
    return (int(max_steps), answer)


def find_number_of_inside_points(input):
    contour = np.array(pipe_path, dtype=np.int32)
...
if __name__ == '__main__':
    ...
    with open(path, 'r') as file:
        input = [char for char in file.read().splitlines()] # part 1 input
        input2 = [list(i) for i in input]   # part 2 input
    ...
    """ part 2 """
    ipts = find_number_of_inside_points(input)
    print(ipts)